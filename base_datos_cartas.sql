...
-- Correlacion para sacar los personajes cuya mascota tenga mas daño que la mascota de su capitan

SELECT p.nombre AS mascota_OP
FROM personajes p
JOIN invocaciones i ON p.id_personaje=i.id_personaje
WHERE i.daño> ANY(
	SELECT i1.daño
	FROM invocaciones i1
    JOIN personajes p1 ON p1.id_personaje=i1.id_personaje
    WHERE p1.id_comandante IS NULL
)AND p.id_comandante IS NOT NULL;